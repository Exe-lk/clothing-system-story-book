...
const ShoppingItems = () => {
  ...
  return (
    ...
    //             <Link href={{pathname: "/kids/#", query: {id: item.id, name: item.des}}}  className={styles.link}>
          ...
          shoppingList.map((item:any, key:any)=>{
            return(
              <div className='col-md-6 col-sm-6 col-lg-4 position-relative d-flex flex-column align-items-center text-capitalize'>
                <Link href={{pathname: "/kids/#", query: {id: item.id, name: item.des}}} className='text-decoration-none text-black'>
                  <Image src={item.name} alt='item' className='img-fluid card-img-top' style={{width:'280px',height:'400px'}}/>
